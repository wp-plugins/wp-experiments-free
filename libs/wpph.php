...
EOT;
	...
	public function license_menu() {
		...
		if(isset($_POST['license'])) {
			$this->set_license($_POST['license']);
...
EOT;
	...
	public function support_view_ticket() {
		...
		if(isset($_POST['reply'])) {
			$comment_url = $this->api_url_base . "/plugin/support/ticket/" . $_GET['tid'] . "/comment";
					...
					echo <<< EOT
						<script type='text/javascript'>
							window.setTimeout(function() {
								window.location = "{$this->pages['view-ticket']}&tid={$_GET['tid']}";
							}, 1000);
						</script>
EOT;
					return;
		...
		$tickets_url = $this->api_url_base . "/plugin/support/ticket/" . $_GET['tid'];
		...
		if(isset($_POST['submit'])) {
			...
			$request_string = array(
				'body' => array(
					...
					"license"=>$this->license,
					'name'=>stripslashes($_POST['ticket']['name']),
					'email'=>stripslashes($_POST['ticket']['email']),
					'subject'=>stripslashes($_POST['ticket']['subject']),
					'message'=>stripslashes($_POST['ticket']['message']),
				),
				'user-agent' => 'WordPress/' . $wp_version . '; ' . get_bloginfo('url')
			);
...
EOT;
	...
	private function takeover_updates() {
		...
		add_filter('pre_set_site_transient_update_plugins', array($this, '__check_for_updates'));
	...
	public function __check_for_updates($checked_data) {
		...
		// if (!$this->testing && empty($checked_data->checked))
			// return $checked_data;
		if(!isset($checked_data->checked[$this->slug])) {
			//plugin isn't being checked
			return $checked_data;
		}
		
		$args = array(
			'slug' => $this->slug,
			'version' => $checked_data->checked[$this->slug]
		);
		...
		if (!is_wp_error($raw_response) && ($raw_response['response']['code'] == 200)) {
			$response_arr = json_decode($raw_response['body'], true);
	...
	public function __plugin_api_call($def, $action, $args) {
		global $wp_version;
